...
public class UserController {
    ...
    public String showProfilePage(Model model, HttpSession session, @RequestParam(defaultValue = "0") int page) {
        ...
        model.addAttribute("gameHistory", gameService.getGameHistory(user.getId(), PageRequest.of(page, 5)));
        model.addAttribute("totalWins", gameService.getTotalWins(user.getId()));
        model.addAttribute("totalLosses", gameService.getTotalLosses(user.getId()));
        return "profile";
    }
